 ...
 * @returns
 */
const updatePomodoro = (pomodoroMsg,secondsLeft) => {
  ...
  if (pomodoroStopped) return;
    ...
    pomodoroMsg.edit(
      getEmbed(
        `Time left: ${new Date(1000 * secondsLeft).toISOString().substr(11, 8)}`
      )
    );
    secondsLeft -= MESSAGE_UPDATE_FREQUENCY;

    setTimeout(() => {
      updatePomodoro(pomodoroMsg,secondsLeft);
...
const startPomodoro = async (msg, seconds) => {
  ...
  updatePomodoro(pomodoroMsg, seconds);
};
...
client.on("message", (msg) => {
  switch (msg.content) {
    case "!start":
      startPomodoro(msg, POMODORO_SESSION_TIME * 60);
